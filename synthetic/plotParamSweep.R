...
get.tenth.bias <- function(){
    load("updateProbData.RData", verbose=TRUE)
    data <- clean.data
    tenths <- 1:10
    total.enc <- 64*50
    seeds <- c(10,500,10)
    for (i in 1:length(tenths)){
        ...
        switch(tenths[i], "1"=ggsave(filename="1UPD.pdf",plot=a), "2"=ggsave(filename="2UPD.pdf", plot=a),
            "3"=ggsave("3UPD.pdf", plot=a), "4"=ggsave("4UPD.pdf", plot=a), "5"=ggsave("5UPD.pdf", plot=a),
            "6"=ggsave("6UPD.pdf", plot=a), "7"=ggsave("7UPD.pdf", plot=a), "8"=ggsave("8UPD.pdf", plot=a),
            "9"=ggsave("9UPD.pdf", plot=a), "10"=ggsave("10UPD.pdf", plot=a), ggsave("error.pdf", plot=a))
   }
