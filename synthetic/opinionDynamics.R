...
param.sweep <- function(seeds=seq(10,500,10), pp=0.5, update=0.5, intern=0.5, con=0.25) {
  ...
  for(i in 1:length(intern)){
    ...
    bias.data <- foreach(num=1:length(seeds), .combine = 'cbind') %dopar% {
        ...
        set.seed(seeds[num])
        initial.graph <- get.expressed.latent.graph(num.agents = 64, prob.connected = con, dir = FALSE)
